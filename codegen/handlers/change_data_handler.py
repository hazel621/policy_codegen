...
@router.post("/hospital_1/change_data/")
async def change_data_handler(agent_id: str, counterparty_role_id:str=None, counterparty_agent_id:str=None, target_role_id: str=None, target_agent_id: str = None, scope: str = None,item: str = None,duty_ref:str=None):
    ...
    state_check = check_state(agent_id=agent_id, action_id= "change_data",scope="hospital_1", target_agent_id=target_agent_id, target_role_id=target_role_id, item=item,duty_ref=duty_ref)
    ...
    print(f"🧭 State Checker: {state_check['message']} for Agent {agent_id} and Action change_data")

    
    
    requester_id = agent_id
    message = f"data of {target_agent_id} changed by doctor_hospital_1"
    ...
    log.end_at = datetime.now()
    log.save()
    return {"status": "success", "action": "change_data"}